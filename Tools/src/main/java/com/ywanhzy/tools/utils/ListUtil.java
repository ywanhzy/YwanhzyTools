...
public class ListUtil {
	...
	/**
     * 求两个集合的非交集   得到newList 中的非公共部分
	 * @param oldList
	 * @param newList
	 * @return
	 */
	public static List getNURetainAll(List oldList,List newList){
		List returnList=null;//返回的集合
		List retainList=null;//交集
		List newnewList=newList;//newList
		
		if (oldList.size()>0&&newList.size()>0) {
			newList.retainAll(oldList);//两个list 的交集
			retainList=newList;
			if (retainList.size()>0) {
				for (int i = 0; i < newnewList.size(); i++) {
					for (int j = 0; j < retainList.size(); j++) {
						if (newnewList.get(i)==retainList.get(j)) {//可能要改一下
							newnewList.remove(i);
						}
					}
				}
			}
			returnList=newnewList;
		}else if (oldList.size()>0&&newList.size()==0) {
			returnList=null;
		}else if (oldList.size()==0&&newList.size()>0) {
			returnList=newList;
		}else if (oldList.size()==0&&newList.size()==0) {
